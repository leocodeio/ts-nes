...
class NESCore {
    constructor(config = {}) {
        ...
        this.config = {
            ...
            ...config,
            ollama: {
                ...types_1.DEFAULT_CONFIG.ollama,
                ...config.ollama,
            },
        };
        this.api = new api_1.OllamaAPI(this.config.ollama);
        this.contextManager = new context_1.ContextManager(this.config.contextWindow, this.config.systemPrompt || undefined);
    }
    /**
    ...
    async getSuggestion(filePath, cursor, originalContent) {
        try {
            ...
            const context = await this.contextManager.createContext(filePath, cursor, originalContent);
            // Create payload for API
            const payload = this.contextManager.createPayload(context, this.config.ollama.model);
            ...
            return new Promise((resolve, reject) => {
                this.api.call(payload, (output) => {
                    try {
                        ...
                        reject(new Error(`Failed to parse suggestion: ${error instanceof Error ? error.message : "Unknown error"}`));
                    }
                }, { stream: true });
            });
        }
            ...
            throw new Error(`Failed to generate suggestion: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Parse AI response into structured edit suggestions
     */
    parseSuggestion(context, aiResponse) {
        ...
        const nextVersionMatch = cleanedResponse.match(/<next-version>(.*?)<\/next-version>/s);
        if (!nextVersionMatch) {
        ...
        const diffResult = (0, diff_1.diffLines)(oldVersion, nextVersionContent, {
        ...
        for (const chunk of diffResult) {
            if (chunk.added || chunk.removed) {
                const textEdit = this.createTextEditFromDiff(chunk, currentLine + lineOffset, diffResult);
                if (textEdit) {
                    suggestions.push({
                        textEdit,
     ...
     */
    removeCodeBlockMarkers(content) {
     ...
     */
    createTextEditFromDiff(chunk, startLine, allChunks) {
    ...
    async healthCheck() {
        ...
        try {
            ...
            errors.push(`Model check failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
