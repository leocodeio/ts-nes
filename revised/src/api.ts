...
import { OllamaConfig, ChatPayload, ChatMessage } from "./types";

/**
 * Response structure from Ollama chat completion API
 */
...
export class OllamaAPI {
   ...
   */
  async pullModel(modelName: string = this.config.model): Promise<void> {
    try {
  ...
  async generateCompletion(
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(
          `Ollama API error: ${axiosError.response?.status} - ${axiosError.message}`
        );
      }
   ...
   */
  private async handleRegularResponse(payload: any): Promise<string> {
  ...
  async debug(): Promise<void> {
    try {
      ...
      if (isHealthy) {
        ...
        console.log(
          `Target Model Available: ${isModelAvailable ? "✅ Yes" : "❌ No"}`
        );

        if (!isModelAvailable) {
          console.log(
            `💡 To pull the model, run: ollama pull ${this.config.model}`
          );
        }
      }
    } catch (error) {
