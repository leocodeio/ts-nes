...
async function testOllamaAPI() {
  ...
  try {
    ...
    // Test a simple generation using the correct method
    const messages = [
      {
        role: "user",
    ...
    const response = await api.generateCompletion(messages);
...
function divide(a: number, b: number): number {
    ...
    // Test context creation using the correct method
    const cursor = [9, 2]; // Inside the divide function
    const context = await contextManager.createContext(testFilePath, cursor);

    if (context && context.current && context.current.content) {
      success("ContextManager created context successfully");
      info(`Context filename: ${context.filename}`);
      info(`Content length: ${context.current.content.length}`);
...
function calculate(operation, a, b) {
  switch (operation) {
    ...
    info(
      `Health check - API: ${health.api}, Model: ${health.model}, Config: ${health.config}`
    );
    ...
    if (health.model) {
      ...
      if (result && result.suggestions && result.suggestions.length > 0) {
        ...
        info(
          `First suggestion: "${firstSuggestion.textEdit.newText.substring(
            0,
            50
          )}..."`
        );
        ...
        warning(
          "No suggestions generated (this might be normal for the test case)"
        );
        return true;
      }
    } else {
...
async function runAllTests() {
  ...
  if (failed === 0) {
    log(
      "\n🎉 All tests passed! The extension appears to be working correctly.",
      "green"
    );
  } else {
